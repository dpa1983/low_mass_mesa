...
!     include 'standard_run_star_extras.inc'
      ...
      subroutine data_for_extra_history_columns(id, id_extra, n, names, vals, ierr)
       ...
       !
         ...
         integer :: k, nz, h1, k_t, k_c
         double precision :: dq00, dqm1, dqsum, T, kap, rho, cp, &
           thermal_diffusivity, gradr, grada, rcl, &
           LHecc, Lr, L_rad, kap_face, alfa, del_m, del_T4, &
           pressure_scale_height, rcl1, rcl2
       ...
       ! find top of He convective core shell (k_c)
         ...
         alfa = s% x_ctrl(1)
         pressure_scale_height = s% P(k_c)/(s% rho(k_c)* &
            s% grav(k_c))
         rcl1 = s% r(k_c) - alfa*pressure_scale_height
         rcl2 = 0.9d0 * s% r(k_c)
         rcl = rcl1
       ! if ( rcl.lt.rcl2 ) rcl = rcl2
         k = k_c + 1
         do while ( s% r(k) > rcl .and. k.lt.nz )
            k = k + 1
         end do
         ...
         Lr = (dqm1*s% L(k) + dq00*s% L(k-1))/dqsum

         LHecc = (1d0 - s% gradT(k)/s% gradr(k))

         names(1) = 'MHecc_ext'
         names(2) = 'MHecc_dot'
         names(3) = 'LHecc_div_L'
         vals(1) = s% star_mass * s% q(k_c)        
         vals(2) = 0.24*(secyer/cgas)*(LHecc*Lr/Msun/T)
         vals(3) = LHecc
      ...
      subroutine pavel_mixing_HB(id, ierr)
       ...
       ! turn off cbm if central He mass fraction is small
         if (s% xa(he4,nz).lt.s% x_ctrl(5)) fac = 0d0
         ...
         do k = k_c, nz 
           ! interpolate values at face
            ...
            if (s% D_mix(k).lt.thermal_diffusivity) then
               s% D_mix(k) = alpha * thermal_diffusivity
