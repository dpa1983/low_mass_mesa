...
! *** dpa ***
      ...
      subroutine goldberg_wind(id, Lsurf, Msurf, Rsurf, Tsurf, w, ierr)
         use const_def, only: dp
         integer, intent(in) :: id
...
! *** dpa *** mass loss is turned on only on the RGB
         if( s% log_surface_luminosity > 1.0 &
           .and. s% center_h1 < 1d-6) then
! Goldberg
!             w = 1.2e-15*(Rsurf/6.9598e10)**3.2
! Judge-Stencel
              w = 4.98e-15*(Rsurf/6.9598e10)**3.2d0/(Msurf/1.989e33)**1.6d0
         else
              w = 0d0
         end if
      ...
      subroutine pavel_mixing_HB(id, ierr)
       ...
       !    fac = fac*(s% xa(he4,nz)/s% x_ctrl(5))**4
